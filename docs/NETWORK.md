...
同源 /ws，HTTPS 使用 WSS。hello 携带 protocolVersion:2、rulesVersion:9、displayName 和可选 token；welcome 返回 token。snapshot 包含 profile、queued、serverNow 和 room。room 包含 code、matchId、seat、participants、ready、rematch、status、state、readyAt、deadlineAt。
...
规则 9：技能准备态 deadlineAt=readyAt，由服务器 tick 自动 attack；客户端不再要求再次点击。坚韧、连续跳过次数和单手封印限制均由共享引擎校验。旧规则 6 对局迁移到等待准备，保留身份与房间码；旧客户端须刷新。

规则 9：calculated 限制每回合一次计算，declinedHands 记录已放弃数值；技能后重新检查配方和计算机会。每次自动攻击后重新开启完整操作计时。仅 PVE 保存最近一局回放；房间凭证按房间号存于同浏览器，新标签输入原码可恢复身份，服务端仍验证凭证与席位。
