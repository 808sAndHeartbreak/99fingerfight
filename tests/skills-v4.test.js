...
test('18 skills are explicitly selectable only for their recipe, with dedicated image assets',()=>{
...
test('buff stacking: foam adds charges, knuckles permanent nonstacking; true attack bonus excludes DOT and props',()=>{
 const s=fixture('foam');s.players[0].foam=1;assert.equal(attack(s).players[0].foam,3);
 const k=fixture('knuckles');k.players[0].knuckles=true;assert.equal(attack(k).players[0].knuckles,true);
...
test('steal preserves own inventory first, discards overflow, copies only digits, and allows dynamic 55',()=>{
 for(const count of [0,2,3]){const s=fixture('steal');s.players[0].props=Array(count).fill('lock');s.players[1].props=['add','sub','double'];s.players[1].turns=1;s.players[1].hands=[5,5];s.players[1].locks=[true,true];s.players[1].knuckles=true;const n=attack(s);assert.deepEqual(n.players[0].props,[...Array(count).fill('lock'),...['add','sub','double'].slice(0,3-count)]);assert.deepEqual(n.players[1].props,[]);assert.deepEqual(n.players[0].hands,[5,5]);assert.deepEqual(n.players[0].locks,[false,false]);assert.equal(n.players[0].knuckles,false);}
});
test('unify preserves statuses and marks, second mark forces win regardless of HP or defense',()=>{
 const s=fixture('unify');s.players[0].nine=1;s.players[0].hp=1;s.players[0].dark=true;s.players[1].hands=[5,5];s.players[1].foam=4;s.players[1].nine=1;const n=attack(s);assert.equal(n.winner,0);assert.equal(n.winReason,'九九归一');assert.equal(n.phase,'over');assert.equal(n.players[0].nine,2);assert.equal(n.players[1].nine,1);assert.equal(n.players[1].hp,99);assert.equal(n.players[1].foam,4);assert.equal(n.players[0].dark,true);assert.deepEqual(n.players.map(p=>p.hands),[[1,1],[1,1]]);assert.match(phaseCue(s,n).title,/九九归一/);assert.deepEqual(legalCommands(n),[]);
});
...
test('random attack bounds, all 31 values reachable, exact replay, immutable input',()=>{
 const values=new Set();
 for(let seed=0;seed<1000;seed++){const s=fixture('drunken');s.rng=seed;const copy=structuredClone(s),n=attack(s);assert.deepEqual(s,copy);assert.deepEqual(attack(s),n);const value=damageEvents(n)[0].raw;assert.ok(value>=10&&value<=40&&Number.isInteger(value));values.add(value);}
 assert.equal(values.size,31);
});
