...
test('only two phases are visible and persistent item receipts add no reading delay',()=>{
 const s=createGame();assert.deepEqual(turnSteps(s).map(x=>x.id),['planning','action']);assert.equal(phaseCue(null,s).duration,0);
