...
test('browser client recovers a room across fresh tab storage using real WebSocket',async()=>{
 ...
 try{
  ...
  const code=first.packet.room.code,token=window.sessionStorage.getItem('ff-token'),socket=first.socket;first.close();await until(()=>socket.readyState===WebSocket.CLOSED);
