...
test('quiet default music preserves explicit saved volume including mute',()=>{
 ...
 try{const fresh=createAudioSettings('music');assert.equal(fresh.values.music,.25);assert.equal(fresh.values.effects,.5);
 ...
 stored='invalid';assert.equal(createAudioSettings('music').values.music,.25);
