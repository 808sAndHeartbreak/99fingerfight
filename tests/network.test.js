...
function harness(options={}) {
 ...
 const a=socket(),b=socket();h.hello(a,{protocolVersion:2,rulesVersion:8,displayName:'一'});h.hello(b,{protocolVersion:2,rulesVersion:8,displayName:'二'});
...
test('matchmaking, cancel, live rename, reconnect grace and rematch agreement',()=>{
 ...
 x.h.disconnect(x.a);x.tick(2000);const c=socket();x.h.hello(c,{token,protocolVersion:2,rulesVersion:8});assert.equal(c.userId,x.a.userId);
 x.h.disconnect(c);x.tick(60001);assert.equal(x.room().state.winner,1);assert.equal(x.room().status,'finished');
 x.h.hello(c,{token,protocolVersion:2,rulesVersion:8});const oldId=x.room().matchId;
...
test('leaving awards the other seat without changing its identity; occupied rooms reject third users',()=>{
 const x=harness();x.ready();const c=socket();x.h.hello(c,{protocolVersion:2,rulesVersion:8});
...
test('room state survives process restart and resume token recovers the same seat',()=>{
 ...
 const restored=new MatchHub({file,now:()=>4000});const c=socket();restored.hello(c,{token,protocolVersion:2,rulesVersion:8});const p=restored.packet(restored.userById(c.userId));assert.equal(p.room.matchId,matchId);assert.equal(p.room.seat,0);
...
test('disconnect clears rematch consent and rejected offline rematch does not trap both seats',()=>{
 ...
 assert.deepEqual(x.room().rematch,[false,false]);
 const resumed=socket();x.h.hello(resumed,{token,protocolVersion:2,rulesVersion:8});
...
test('HTTP health, static source isolation, and live WebSocket hello',async()=>{
 ...
 await new Promise((resolve,reject)=>{const ws=new WebSocket(`ws://127.0.0.1:${port}/ws`);ws.on('open',()=>ws.send(JSON.stringify({type:'hello',protocolVersion:2,rulesVersion:8,displayName:'网络测试'})));ws.on('message',data=>{if(JSON.parse(data).type==='snapshot'){ws.close();resolve();}});ws.on('error',reject);});
...
test('two real WebSocket clients complete a match with identical revisions and result',async()=>{
 ...
 const client=()=>new Promise((resolve,reject)=>{const ws=new WebSocket(url), c={ws,packet:null,seq:0,pending:new Map()};
  ws.on('open',()=>ws.send(JSON.stringify({type:'hello',protocolVersion:2,rulesVersion:8,displayName:'联机测试'})));
...
test('v4 status counters survive public snapshots, duplicate attacks, and reconnect',()=>{
 ...
 const token=x.b.messages.find(m=>m.type==='welcome').token;x.h.disconnect(x.b);const b=socket();x.h.hello(b,{token,protocolVersion:2,rulesVersion:8});assert.deepEqual(b.messages.at(-1).room.state,x.h.publicState(after));
 x.tick(5000);assert.equal(r.state.active,0);assert.equal(r.state.players[1].skip,2);
});

test('initial versus introduction reserves time before the first planning clock',()=>{
 ...
 x.tick(1399);assert.equal(x.room().state.phase,'start');x.tick(1);assert.equal(x.room().state.phase,'planning');
 assert.equal(x.room().deadlineAt-x.room().readyAt,20000);
});
...
test('unnamed online seats are distinct and submitted names survive reconnect',()=>{
 ...
 const token=x.b.messages.find(m=>m.type==='welcome').token;
 x.h.disconnect(x.b);const b=socket();x.h.hello(b,{token,displayName:'',protocolVersion:2,rulesVersion:8});
...
test('v5 buffs transfer authoritatively, duplicate requests do not stack twice, reconnect retains counters',()=>{
 ...
 assert.equal(after.players[0].knuckles,5);assert.equal(after.players[0].peace,5);assert.equal(after.players[1].knuckles,0);assert.equal(after.players[1].peace,0);assert.equal(after.players[1].foam,5);assert.equal(after.players[1].poison,4);
 assert.equal(after.players[1].echo,true);assert.equal(after.players[1].nine,1);
 const token=x.b.messages.find(m=>m.type==='welcome').token;x.h.disconnect(x.b);const b=socket();x.h.hello(b,{token,protocolVersion:2,rulesVersion:8});assert.deepEqual(b.messages.at(-1).room.state,x.h.publicState(r.state));
});

test('wine use is idempotent online and empty item phase advances on the shared one-second deadline',()=>{
 const x=harness();x.ready();x.tick(1100);const r=x.room();r.state.phase='planning';r.state.players[0].props=['wine'];r.state.players[0].hands=[1,2];
 const m={id:'wine-once',op:'command',matchId:r.matchId,command:{type:'prop',slot:0,target:0,revision:r.state.revision}};
 x.h.request(x.a,m);const revision=r.state.revision;x.h.request(x.a,m);assert.equal(r.state.revision,revision);assert.equal(r.state.players[0].wine,1);assert.equal(r.deadlineAt-r.readyAt,1000);
 x.tick(2001);assert.equal(r.state.phase,'action');assert.equal(r.state.players[0].wine,1);
 const token=x.a.messages.find(m=>m.type==='welcome').token;x.h.disconnect(x.a);const a=socket();x.h.hello(a,{token,protocolVersion:2,rulesVersion:8});assert.deepEqual(a.messages.at(-1).room.state,x.h.publicState(r.state));
});

test('forged skill auto-fires at server readiness without a second player request',()=>{
 const x=harness();x.ready();x.tick(1100);const r=x.room();r.state.phase='synthesis';r.state.players[0].hands=[2,2];
 x.req(x.a,'command',{matchId:r.matchId,command:{type:'forge',weapon:'scissors',revision:r.state.revision}});
 assert.equal(r.deadlineAt,r.readyAt);const revision=r.state.revision;
 ...
 x.h.disconnect(x.a);const restored=socket();x.h.hello(restored,{token,protocolVersion:2,rulesVersion:8});x.req(restored,'join',{code});
 ...
 const intruder=socket();x.h.hello(intruder,{protocolVersion:2,rulesVersion:8});assert.throws(()=>x.req(intruder,'join',{code}));assert.equal(x.room().matchId,id);
 });
