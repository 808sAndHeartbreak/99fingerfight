...
function harness(options={}) {
 ...
 const a=socket(),b=socket();h.hello(a,{protocolVersion:1,rulesVersion:4,displayName:'一'});h.hello(b,{protocolVersion:1,rulesVersion:4,displayName:'二'});
...
test('matchmaking, cancel, live rename, reconnect grace and rematch agreement',()=>{
 ...
 x.h.disconnect(x.a);x.tick(2000);const c=socket();x.h.hello(c,{token,protocolVersion:1,rulesVersion:4});assert.equal(c.userId,x.a.userId);
 x.h.disconnect(c);x.tick(60001);assert.equal(x.room().state.winner,1);assert.equal(x.room().status,'finished');
 x.h.hello(c,{token,protocolVersion:1,rulesVersion:4});const oldId=x.room().matchId;
...
test('leaving awards the other seat without changing its identity; occupied rooms reject third users',()=>{
 const x=harness();x.ready();const c=socket();x.h.hello(c,{protocolVersion:1,rulesVersion:4});
...
test('room state survives process restart and resume token recovers the same seat',()=>{
 ...
 const restored=new MatchHub({file,now:()=>4000});const c=socket();restored.hello(c,{token,protocolVersion:1,rulesVersion:4});const p=restored.packet(restored.userById(c.userId));assert.equal(p.room.matchId,matchId);assert.equal(p.room.seat,0);
...
test('disconnect clears rematch consent and rejected offline rematch does not trap both seats',()=>{
 ...
 assert.deepEqual(x.room().rematch,[false,false]);
 const resumed=socket();x.h.hello(resumed,{token,protocolVersion:1,rulesVersion:4});
...
test('HTTP health, static source isolation, and live WebSocket hello',async()=>{
 ...
 await new Promise((resolve,reject)=>{const ws=new WebSocket(`ws://127.0.0.1:${port}/ws`);ws.on('open',()=>ws.send(JSON.stringify({type:'hello',protocolVersion:1,rulesVersion:4,displayName:'网络测试'})));ws.on('message',data=>{if(JSON.parse(data).type==='snapshot'){ws.close();resolve();}});ws.on('error',reject);});
...
test('two real WebSocket clients complete a match with identical revisions and result',async()=>{
 ...
 const client=()=>new Promise((resolve,reject)=>{const ws=new WebSocket(url), c={ws,packet:null,seq:0,pending:new Map()};
  ws.on('open',()=>ws.send(JSON.stringify({type:'hello',protocolVersion:1,rulesVersion:4,displayName:'联机测试'})));
...
test('old rule clients cannot join a new-rule server',()=>{
 const h=new MatchHub(),s=socket();assert.throws(()=>h.hello(s,{protocolVersion:1,rulesVersion:3}),/刷新/);assert.equal(s.closed,true);assert.equal(h.users.size,0);
});
...
test('v4 status counters survive public snapshots, duplicate attacks, and reconnect',()=>{
 ...
 const token=x.b.messages.find(m=>m.type==='welcome').token;x.h.disconnect(x.b);const b=socket();x.h.hello(b,{token,protocolVersion:1,rulesVersion:4});assert.deepEqual(b.messages.at(-1).room.state,x.h.publicState(after));
...
test('production opening cut-in is protected before planning begins',()=>{
 ...
 assert.equal(x.room().readyAt,4380);assert.equal(x.room().deadlineAt,5380);
 x.tick(3200);assert.equal(x.room().state.phase,'start');
 assert.throws(()=>x.req(x.a,'command',{matchId:x.room().matchId,command:{type:'advance',revision:0}}),/阶段切换/);
 x.tick(1181);assert.equal(x.room().state.phase,'planning');
