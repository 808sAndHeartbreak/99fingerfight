...
function harness(options={}) {
 ...
 const a=socket(),b=socket();h.hello(a,{protocolVersion:2,rulesVersion:RULES_VERSION,displayName:'一'});h.hello(b,{protocolVersion:2,rulesVersion:RULES_VERSION,displayName:'二'});
...
test('server binds actor, rejects stale/mismatched commands and deduplicates accepted command IDs',()=>{
 ...
 assert.throws(()=>x.req(x.b,'command',{matchId:room.matchId,command:{type:'add',hand:0,targetHand:0,actor:0,revision}}),/轮到/);
 assert.throws(()=>x.req(x.a,'command',{matchId:'wrong',command:{type:'add',hand:0,targetHand:0,revision}}),/更新/);
 const m={id:'stable',op:'command',matchId:room.matchId,command:{type:'add',hand:0,targetHand:0,actor:1,revision}};
...
test('matchmaking, cancel, live rename, reconnect grace and rematch agreement',()=>{
 ...
 x.h.disconnect(x.a);x.tick(2000);const c=socket();x.h.hello(c,{token,protocolVersion:2,rulesVersion:RULES_VERSION});assert.equal(c.userId,x.a.userId);
 x.h.disconnect(c);x.tick(60001);assert.equal(x.room().state.winner,1);assert.equal(x.room().status,'finished');
 x.h.hello(c,{token,protocolVersion:2,rulesVersion:RULES_VERSION});const oldId=x.room().matchId;
...
test('leaving awards the other seat without changing its identity; occupied rooms reject third users',()=>{
 const x=harness();x.ready();const c=socket();x.h.hello(c,{protocolVersion:2,rulesVersion:RULES_VERSION});
...
test('room state survives process restart and resume token recovers the same seat',()=>{
 ...
 const restored=new MatchHub({file,now:()=>4000});const c=socket();restored.hello(c,{token,protocolVersion:2,rulesVersion:RULES_VERSION});const p=restored.packet(restored.userById(c.userId));assert.equal(p.room.matchId,matchId);assert.equal(p.room.seat,0);
...
test('disconnect clears rematch consent and rejected offline rematch does not trap both seats',()=>{
 ...
 assert.deepEqual(x.room().rematch,[false,false]);
 const resumed=socket();x.h.hello(resumed,{token,protocolVersion:2,rulesVersion:RULES_VERSION});
...
test('HTTP health, static source isolation, and live WebSocket hello',async()=>{
 ...
 await new Promise((resolve,reject)=>{const ws=new WebSocket(`ws://127.0.0.1:${port}/ws`);ws.on('open',()=>ws.send(JSON.stringify({type:'hello',protocolVersion:2,rulesVersion:RULES_VERSION,displayName:'网络测试'})));ws.on('message',data=>{if(JSON.parse(data).type==='snapshot'){ws.close();resolve();}});ws.on('error',reject);});
...
test('two real WebSocket clients complete a match with identical revisions and result',async()=>{
 ...
 const client=()=>new Promise((resolve,reject)=>{const ws=new WebSocket(url), c={ws,packet:null,seq:0,pending:new Map()};
  ws.on('open',()=>ws.send(JSON.stringify({type:'hello',protocolVersion:2,rulesVersion:RULES_VERSION,displayName:'联机测试'})));
 ...
 try {
  ...
  while(app.hub.rooms.values().next().value.state.winner===null&&steps++<1200){
   ...
   if(room.state.phase==='start'||room.state.players[room.state.active].weapon){now=room.readyAt+1;app.hub.tick();await new Promise(r=>setTimeout(r,5));continue;}
   ...
   await actor.request('command',{matchId:room.matchId,command});await new Promise(r=>setTimeout(r,5));
  }
...
test('old rule clients cannot join a new-rule server',()=>{
 const h=new MatchHub(),s=socket();assert.throws(()=>h.hello(s,{protocolVersion:2,rulesVersion:5}),/刷新/);assert.equal(s.closed,true);assert.equal(h.users.size,0);
});
test('old persisted games return to preparation without carrying removed items',()=>{
 const dir=mkdtempSync(join(tmpdir(),'ff-v3-'));
 try{const file=join(dir,'state.json'),x=harness({file});x.ready();const code=x.room().code;x.room().state.rulesVersion=5;x.room().state.players[0].props=['heal'];x.h.save();
...
test('v4 status counters survive public snapshots, duplicate attacks, and reconnect',()=>{
 ...
 const token=x.b.messages.find(m=>m.type==='welcome').token;x.h.disconnect(x.b);const b=socket();x.h.hello(b,{token,protocolVersion:2,rulesVersion:RULES_VERSION});assert.deepEqual(b.messages.at(-1).room.state,x.h.publicState(after));
...
test('unnamed online seats are distinct and submitted names survive reconnect',()=>{
 ...
 const token=x.b.messages.find(m=>m.type==='welcome').token;
 x.h.disconnect(x.b);const b=socket();x.h.hello(b,{token,displayName:'',protocolVersion:2,rulesVersion:RULES_VERSION});
...
test('v5 buffs transfer authoritatively, duplicate requests do not stack twice, reconnect retains counters',()=>{
 const x=harness();x.ready();x.tick(1100);const r=x.room();r.state.phase='action';r.state.calculated=true;r.state.players[0].weapon='steal';
 Object.assign(r.state.players[0],{knuckles:2,peace:2,peaceSince:0});Object.assign(r.state.players[1],{knuckles:3,peace:4,foam:6,weak:5,poison:5,echo:true,mirror:true,nine:1});
 const m={id:'v5-steal',op:'command',matchId:r.matchId,command:{type:'attack',revision:r.state.revision}};
 ...
 const token=x.b.messages.find(m=>m.type==='welcome').token;x.h.disconnect(x.b);const b=socket();x.h.hello(b,{token,protocolVersion:2,rulesVersion:RULES_VERSION});assert.deepEqual(b.messages.at(-1).room.state,x.h.publicState(r.state));
});

test('wine use is idempotent online and empty inventory preserves remaining turn time',()=>{
 const x=harness();x.ready();x.tick(1100);const r=x.room();r.state.phase='action';r.state.players[0].props=['wine'];r.state.players[0].hands=[1,2];
 ...
 const token=x.a.messages.find(m=>m.type==='welcome').token;x.h.disconnect(x.a);const a=socket();x.h.hello(a,{token,protocolVersion:2,rulesVersion:RULES_VERSION});assert.deepEqual(a.messages.at(-1).room.state,JSON.parse(JSON.stringify(x.h.publicState(r.state))));
});

test('forged skill auto-fires at server readiness without a second player request',()=>{
 const x=harness();x.ready();x.tick(1100);const r=x.room();r.state.phase="action";r.state.players[0].hands=[2,2];
 x.req(x.a,'command',{matchId:r.matchId,command:{type:'forge',weapon:'scissors',revision:r.state.revision}});
 assert.ok(r.deadlineAt>r.readyAt);const revision=r.state.revision;
 x.tick(1);assert.equal(r.state.players[1].hp,94);assert.equal(r.state.players[0].weapon,null);assert.equal(r.state.revision,revision+1);
 const hp=r.state.players[1].hp;x.tick(1);assert.equal(r.state.players[1].hp,hp);
});
 ...
 x.h.disconnect(x.a);const restored=socket();x.h.hello(restored,{token,protocolVersion:2,rulesVersion:RULES_VERSION});x.req(restored,'join',{code});
 ...
 const intruder=socket();x.h.hello(intruder,{protocolVersion:2,rulesVersion:RULES_VERSION});assert.throws(()=>x.req(intruder,'join',{code}));assert.equal(x.room().matchId,id);
 });

test('expired requests cannot extend a turn, and end requests remain idempotent',()=>{
 const x=harness();x.ready();const r=x.room();r.readyAt=0;r.deadlineAt=0;
 assert.throws(()=>x.req(x.a,'command',{matchId:r.matchId,command:{type:'add',hand:0,targetHand:0,revision:r.state.revision}}),/时间已到/);
 r.state.calculated=true;
 const m={id:'end-once',op:'command',matchId:r.matchId,command:{type:'end',revision:r.state.revision}};
 x.h.request(x.a,m);const revision=r.state.revision;x.h.request(x.a,m);assert.equal(r.state.revision,revision);assert.equal(r.state.active,1);
});

test('manual early end is accepted without a timeout flag; server timeout still advances',()=>{
 const x=harness();x.ready();const r=x.room();r.readyAt=0;
 x.req(x.a,'command',{matchId:r.matchId,command:{type:'end',revision:r.state.revision}});
