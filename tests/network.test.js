...
function harness(options={}) {
 ...
 const a=socket(),b=socket();h.hello(a,{protocolVersion:2,rulesVersion:12,displayName:'一'});h.hello(b,{protocolVersion:2,rulesVersion:12,displayName:'二'});
...
test('matchmaking, cancel, live rename, reconnect grace and rematch agreement',()=>{
 ...
 x.h.disconnect(x.a);x.tick(2000);const c=socket();x.h.hello(c,{token,protocolVersion:2,rulesVersion:12});assert.equal(c.userId,x.a.userId);
 x.h.disconnect(c);x.tick(60001);assert.equal(x.room().state.winner,1);assert.equal(x.room().status,'finished');
 x.h.hello(c,{token,protocolVersion:2,rulesVersion:12});const oldId=x.room().matchId;
...
test('leaving awards the other seat without changing its identity; occupied rooms reject third users',()=>{
 const x=harness();x.ready();const c=socket();x.h.hello(c,{protocolVersion:2,rulesVersion:12});
...
test('room state survives process restart and resume token recovers the same seat',()=>{
 ...
 const restored=new MatchHub({file,now:()=>4000});const c=socket();restored.hello(c,{token,protocolVersion:2,rulesVersion:12});const p=restored.packet(restored.userById(c.userId));assert.equal(p.room.matchId,matchId);assert.equal(p.room.seat,0);
...
test('disconnect clears rematch consent and rejected offline rematch does not trap both seats',()=>{
 ...
 assert.deepEqual(x.room().rematch,[false,false]);
 const resumed=socket();x.h.hello(resumed,{token,protocolVersion:2,rulesVersion:12});
...
test('HTTP health, static source isolation, and live WebSocket hello',async()=>{
 ...
 await new Promise((resolve,reject)=>{const ws=new WebSocket(`ws://127.0.0.1:${port}/ws`);ws.on('open',()=>ws.send(JSON.stringify({type:'hello',protocolVersion:2,rulesVersion:12,displayName:'网络测试'})));ws.on('message',data=>{if(JSON.parse(data).type==='snapshot'){ws.close();resolve();}});ws.on('error',reject);});
...
test('two real WebSocket clients complete a match with identical revisions and result',async()=>{
 ...
 const client=()=>new Promise((resolve,reject)=>{const ws=new WebSocket(url), c={ws,packet:null,seq:0,pending:new Map()};
  ws.on('open',()=>ws.send(JSON.stringify({type:'hello',protocolVersion:2,rulesVersion:12,displayName:'联机测试'})));
...
test('v4 status counters survive public snapshots, duplicate attacks, and reconnect',()=>{
 ...
 const token=x.b.messages.find(m=>m.type==='welcome').token;x.h.disconnect(x.b);const b=socket();x.h.hello(b,{token,protocolVersion:2,rulesVersion:12});assert.deepEqual(b.messages.at(-1).room.state,x.h.publicState(after));
...
test('unnamed online seats are distinct and submitted names survive reconnect',()=>{
 ...
 const token=x.b.messages.find(m=>m.type==='welcome').token;
 x.h.disconnect(x.b);const b=socket();x.h.hello(b,{token,displayName:'',protocolVersion:2,rulesVersion:12});
...
test('v5 buffs transfer authoritatively, duplicate requests do not stack twice, reconnect retains counters',()=>{
 ...
 const token=x.b.messages.find(m=>m.type==='welcome').token;x.h.disconnect(x.b);const b=socket();x.h.hello(b,{token,protocolVersion:2,rulesVersion:12});assert.deepEqual(b.messages.at(-1).room.state,x.h.publicState(r.state));
});
...
test('wine use is idempotent online and empty inventory preserves remaining turn time',()=>{
 ...
 const token=x.a.messages.find(m=>m.type==='welcome').token;x.h.disconnect(x.a);const a=socket();x.h.hello(a,{token,protocolVersion:2,rulesVersion:12});assert.deepEqual(a.messages.at(-1).room.state,JSON.parse(JSON.stringify(x.h.publicState(r.state))));
});
...
test('forged skill auto-fires at server readiness without a second player request',()=>{
 ...
 x.h.disconnect(x.a);const restored=socket();x.h.hello(restored,{token,protocolVersion:2,rulesVersion:12});x.req(restored,'join',{code});
 ...
 const intruder=socket();x.h.hello(intruder,{protocolVersion:2,rulesVersion:12});assert.throws(()=>x.req(intruder,'join',{code}));assert.equal(x.room().matchId,id);
 });
...
test('manual end is rejected before calculation, including fake timeout fields, while server timeout still advances',()=>{
 const x=harness();x.ready();const r=x.room();r.readyAt=0;
 const before=structuredClone(r.state);
 assert.throws(()=>x.req(x.a,'command',{matchId:r.matchId,command:{type:'end',timeout:true,revision:r.state.revision}}),/计算一次/);
 assert.deepEqual(r.state,before);
 r.deadlineAt=0;x.h.tick();assert.equal(r.state.active,1);
});
