...
test('each A/B/C item has exact 4/2/1 ticket weight, with no missing items', () => {
  ...
  assert.equal(PROP_WEIGHT_TOTAL,28);
  ...
  assert.throws(()=>propForTicket(-1));assert.throws(()=>propForTicket(28));
});

test('weighted repeated draws are deterministic, inventory capped, input immutable', () => {
  const s=createGame(123);s.phase='action';s.players[0].props=['greed'];s.players[1].props=['wine','add','sub'];
