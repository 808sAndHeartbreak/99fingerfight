...
test('decoded music loops once, fades and resumes position without duplicate sources',async()=>{
 ...
 const events={},sources=[],ramps=[];let context,requests=0;
 class Context{
  ...
  decodeAudioData(){return Promise.resolve({duration:137.142857});}
 ...
 try{
  ...
  document.hidden=false;events.visibilitychange();assert.equal(sources.length,2);assert.ok(Math.abs(sources[1].offset-12.857143)<.00001);assert.deepEqual(ramps.at(-1),[.14,.18]);
