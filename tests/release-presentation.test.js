...
test('nine mark acquisition and forced win have separate pacing and outcomes',()=>{
 ...
 s.players[0].nine=1;const last=run(s,{type:'attack'});assert.equal(actionDuration(s,last,{type:'attack'}),4400);assert.equal(actionBeats(s,last,{type:'attack'}).at(-1).label,'蓝方第二次使用归一，九九归一获胜');
});
...
test('all production illustrations exist with nonempty files and no placeholder aliases',()=>{
 const files=[...WEAPONS.map(w=>w.image),...Object.values(PROPS).map(p=>p.image),'ink-mono/nine-seal.webp','ink-mono/resilience.webp'];assert.equal(new Set(files).size,35);
