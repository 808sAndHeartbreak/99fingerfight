...
test('all production illustrations exist with nonempty files and no placeholder aliases',()=>{
 const files=[...WEAPONS.map(w=>w.image),...Object.values(PROPS).map(p=>p.image),'ink-mono/nine-seal.webp','ink-mono/resilience.webp'];assert.equal(new Set(files).size,34);
