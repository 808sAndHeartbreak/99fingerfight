...
test('all AI levels calculate against zero when it avoids an empty-turn penalty',()=>{
 for(const level of ['easy','advanced','expert','master'])for(const hands of [[0,0],[0,1],[1,0]]){
...
test('zero remains a useful echo target when it copies a winning combination',()=>{
 ...
 for(const level of ['advanced','expert','master']){const c=chooseCommand(s,level);assert.equal(c.type,'add');assert.equal(c.hand,0);assert.deepEqual(applyCommand(s,c).players[0].hands,[9,9]);}
});
