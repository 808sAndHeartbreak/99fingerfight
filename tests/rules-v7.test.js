...
const act=s=>{s.phase='action';s.calculated=true;s.players[s.active].weapon='foam';const n=run(s,{type:'attack'});return n.winner===null&&n.active===s.active?run(n,{type:'end'}):n;};
...
test('resilience prevents both hostile and self skips, transfers through steal, and survives unify',()=>{
 for(const id of ['taser','buddha','serious']){
  let s=createGame();s.phase='action';s.calculated=true;s.players[0].weapon=id;s.players[0].resilience=5;s.players[1].resilience=5;
  const n=run(s,{type:'attack'});assert.equal(n.players[0].skip,0);assert.equal(n.players[1].skip,0);assert.equal(n.skipping,false);
 }
 let s=createGame();s.phase='action';s.calculated=true;s.players[0].weapon='steal';s.players[0].skip=3;s.players[1].resilience=4;
 ...
 n=run(n,{type:'end'});n.phase='action';n.calculated=true;n.players[1].weapon='unify';n.players[1].resilience=2;n=run(n,{type:'attack'});n=run(n,{type:'end'});assert.equal(n.players[1].resilience,1);
});
test('declining a recipe ends turn without changing hands or earning resilience; no-target end does not count',()=>{
 let s=createGame();s.phase="action";s.calculated=true;s.players[0].hands=[5,5];s.players[0].skippedTurns=1;
 let n=run(s,{type:'end'});assert.equal(n.active,1);assert.deepEqual(n.players[0].hands,[5,5]);assert.equal(n.players[0].skippedTurns,0);assert.equal(n.players[0].resilience,0);
 s=createGame();s.phase='action';s.players[0].locks=[true,true];n=run(s,{type:'end'});assert.equal(n.active,1);assert.equal(n.players[0].skippedTurns,0);
});

test('seal permits at most one hand per player, rejects repeat without consumption, and respects resilience',()=>{
 let s=createGame();s.phase='action';s.players[0].props=['lock','lock','lock'];s.players[1].resilience=5;
