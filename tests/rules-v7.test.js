...
const act=s=>{s.phase='action';s.calculated=true;s.players[s.active].weapon='foam';return run(s,{type:'attack'});};
...
test('resilience prevents both hostile and self skips, transfers through steal, and survives unify',()=>{
 ...
 n.phase='action';n.calculated=true;n.players[1].weapon='unify';n.players[1].resilience=2;n=run(n,{type:'attack'});assert.equal(n.players[1].resilience,1);
});
test('declining a recipe ends turn without changing hands or earning resilience; no-target end does not count',()=>{
 let s=createGame();s.phase='synthesis';s.calculated=true;s.players[0].hands=[5,5];s.players[0].skippedTurns=1;
