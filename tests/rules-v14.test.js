...
test('retired item is absent and old rules reject before applying commands',()=>{assert.equal(PROPS.adrenaline,undefined);assert.equal(PROP_IDS.length,12);const s=fresh();assert.equal('adrenaline' in s.players[0],false);assert.throws(()=>run({...s,rulesVersion:RULES_VERSION-1},{type:'end'}),/版本/);});
for(const difficulty of ['easy','advanced','expert','master'])test(`${difficulty} avoids fatal empty turn when a zero target calculation is available`,()=>{const s=fresh();s.players[0].hp=5;s.players[1].hands=[0,0];const c=chooseCommand(s,difficulty);assert.equal(c.type,'add');});
