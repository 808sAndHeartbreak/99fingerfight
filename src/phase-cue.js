...
export function phaseCue(old, next, participants) {
  if (next.winner !== null) return old?.winner === next.winner ? null : {kind:"finish", owner:next.winner, title:next.winReason || "胜负已定", label:"决 着", detail:`${playerName(participants, next.winner)} 获胜`, duration:2600};
  ...
  return {kind:next.phase,owner:next.active,title,label,detail:`${playerName(participants,next.active)} · ${detail}`,duration:next.phase === "start" ? 3200 : 1800};
}
