import {DEFAULT_MATCH_OPTIONS,matchSummary} from './match-options.js';
...
export class OnlineClient {
 constructor(onChange,onStatus) {this.options={...DEFAULT_MATCH_OPTIONS};this.onChange=onChange;this.onStatus=onStatus;this.pending=new Map();this.status='offline';this.packet=null;this.retry=0;this.closed=false;}
 connect(){
 ...
 close(){this.closed=true;clearTimeout(this.timer);const socket=this.socket;this.socket=null;this.status='offline';for(const p of this.pending.values()){clearTimeout(p.timer);p.reject(new Error('已关闭联机连接'));}this.pending.clear();socket?.close();}
}
export function onlineMarkup(net){
 ...
 let body='';
 if(r){
  body=`<div class="room-code"><span>房间码</span><strong>${r.code}</strong><button id="copy-room" class="secondary">复制邀请</button></div><div class="room-players">${[0,1].map(i=>{const player=r.participants[i],present=player&&!player.departed;return `<div class="room-seat seat-${i} ${present&&player.connected?'present':''} ${present&&(r.status==='finished'?r.rematch[i]:r.ready[i])?'ready':''}"><small>PLAYER 0${i+1}${i===r.seat?' / 你':''}</small><b title="${present?e(player.displayName):'等待玩家'}">${present?e(player.displayName):'等待玩家'}</b><span>${present?player.connected?(r.status==='finished'?(r.rematch[i]?'已准备再战':'等待准备'):r.ready[i]?'已准备':r.status==='playing'?'对局中':'未准备'):'等待重连':player?.departed?'已离开房间':'分享房间码邀请加入'}</span></div>`;}).join('')}</div>`;
  body+=`<section class="room-settings"><div class="room-settings-heading"><h3>对局规则</h3><small>${r.status==='playing'?'对局中锁定':r.seat===0?'房主设置 · 修改后双方重新准备':'由房主设置'}</small></div>${matchSettings({options:r.options,scope:'room',disabled:r.seat!==0||r.status==='playing'||!connected||!!net.busy})}</section>`;
  body+=r.status==='waiting'?`<button id="online-ready" class="primary" ${disabled}>${r.ready[r.seat]?'取消准备':'准备对战'} </button><p class="lobby-hint">${!peer||peer.departed?'复制邀请链接给好友，双方准备后开局':!peer.connected?'等待对手恢复连接':r.ready[r.seat]?'你已就绪，等待对手准备':'双方准备后开始'}</p>`:r.status==='finished'?`<button id="online-rematch" class="primary" ${disabled} ${r.rematch[r.seat]||!canRematch?'disabled':''}>${peer?.departed?'对手已离开':!canRematch?'等待对手返回':r.rematch[r.seat]?'已准备 · 等待对手':'准备再战'}</button><p class="lobby-hint">${peer?.departed?'对手已离开，返回大厅开始新的对战。':r.rematch[1-r.seat]?'对手已准备，你准备后即可开始。':'留在当前房间，双方准备后开始新对局。'}</p>`:'<p class="lobby-hint">对局仍在继续，返回战场即可出手。</p><button class="primary" data-close>返回对局 </button>';
  body+=`<button id="online-leave" class="secondary lobby-back" ${disabled}>${r.status==='playing'?'离开并认输':'离开房间'}</button>`;
 } else if(p?.queued)body=`<div class="match-search" role="status"><div class="search-orbit" aria-hidden="true">VS</div><b>正在寻找对手</b><span>${matchSummary(p.queueOptions||net.options)}<br>匹配相同规则的对手，找到后自动开局。</span></div><button id="online-cancel" class="secondary" ${disabled}>取消匹配</button>`;
 else if(net.view==='join'||invite)body=`<form id="join-form"><label for="room-code">输入好友的房间码</label><div class="name-entry"><input id="room-code" name="code" maxlength="6" value="${e(net.joinCode||invite)}" placeholder="六位房间码" autocomplete="off" autocapitalize="characters" spellcheck="false" required><button class="secondary" ${disabled}>加入房间</button></div></form><button id="online-options" class="secondary lobby-back"> 联机选项</button>`;
 else body=`<section class="lobby-setup"><h3>本次对战规则</h3>${matchSettings({options:net.options,scope:'online',disabled:!connected||!!net.busy})}</section><div class="online-options"><button id="online-queue" class="menu-choice" ${disabled}><span>开始匹配<small>只匹配相同规则的玩家</small></span></button><button id="online-create" class="menu-choice" ${disabled}><span>创建房间<small>邀请好友，准备后开局</small></span><b>＋</b></button><button id="online-join" class="menu-choice" ${disabled}><span>加入房间<small>输入好友的房间码</small></span></button></div>`;
 const connection=connected?'已连接':net.status==='outdated'?'版本已更新，请刷新页面':net.status==='replaced'?'此身份已在其他页面打开':'连接中 · 自动重试';
 return `<div class="dialog-body online-lobby ${r?'has-room':p?.queued?'is-searching':'lobby-home'}">${!p?.queued&&r?.status!=="waiting"?`<button class="dialog-close" data-close aria-label="${r?.status==='finished'?'返回结算':r?.state?'返回对局':'返回菜单'}">×</button>`:""}<header class="lobby-header"><small class="scene-kicker">FINGER FIGHT / ONLINE</small><h2>${r?'对战房间':p?.queued?'寻找对手':net.view==='join'||invite?'加入房间':'联机对战'}</h2><p class="network-state" role="status"><i class="status-dot ${connected?'connected':''}"></i>${connection}${net.status==='replaced'?'<button id="online-reconnect">重新连接</button>':''}</p></header><aside class="lobby-identity"><form id="name-form"><label for="online-name">你的名字 <small>最多 10 个字符</small></label><div class="name-entry"><input id="online-name" name="name" maxlength="200" value="${e(net.nameDraft ?? (p?.profile?.displayName||read('ff-name','localStorage')||''))}" placeholder="输入昵称（可选）" autocomplete="nickname"><button class="secondary" ${disabled}>保存</button></div></form></aside><section class="lobby-content" aria-label="${r?'房间状态':'联机选项'}">${body}<p class="online-error" id="online-error" role="status">${e(net.error||'')}</p>${!r&&!p?.queued?'<button id="online-back" class="secondary lobby-back">返回模式选择</button>':''}</section></div>`;
}
