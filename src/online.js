...
export function onlineMarkup(net){
 ...
 return `<div class="dialog-body online-lobby ${r?'has-room':p?.queued?'is-searching':'lobby-home'}">${!p?.queued&&r?.status!=="waiting"?`<button class="dialog-close" data-close aria-label="${r?.state?'返回对局':'返回菜单'}">×</button>`:""}<header class="lobby-header"><small class="scene-kicker">FINGER FIGHT / ONLINE</small><h2>${r?'对战房间':p?.queued?'寻找对手':net.view==='join'||invite?'加入房间':'联机对战'}</h2><p class="network-state" role="status"><i class="status-dot ${connected?'connected':''}"></i>${connection}${net.status==='replaced'?'<button id="online-reconnect">重新连接</button>':''}</p></header><aside class="lobby-identity"><div class="lobby-portrait" aria-hidden="true"></div><form id="name-form"><label for="online-name">你的名字 <small>最多 10 个字符</small></label><div class="name-entry"><input id="online-name" name="name" maxlength="200" value="${e(net.nameDraft ?? (p?.profile?.displayName||read('ff-name','localStorage')||''))}" placeholder="输入昵称（可选）" autocomplete="nickname"><button class="secondary" ${disabled}>保存</button></div></form></aside><section class="lobby-content" aria-label="${r?'房间状态':'联机选项'}">${body}<p class="online-error" id="online-error" role="status">${e(net.error||'')}</p>${!r&&!p?.queued?'<button id="online-back" class="text-button">返回模式选择</button>':''}</section></div>`;
}
