export const WEAPONS = Object.freeze([
  ...
  {"id": "unify", "recipe": [9, 9], "name": "归一", "damage": 0, "detail": "清除自己的全部减益，双方双手归 [1]，获得一次“归一”标记，累计使用两次后立即获胜。"},
].map(w=>Object.freeze({...w,image:`ink-mono/${w.id}.webp`})));
export const MAX_HP = 99;
export const PROPS = Object.freeze({
  wine: {name:"酒", detail:"下一次攻击伤害 +10", rules:"只强化下一次有直接伤害的攻击的首段伤害。可以永久叠加多次酒，下次攻击时自动全部消耗。无直接伤害的技能不消耗；攻击被挡住仍会消耗。", target:"self", image:"ink-mono/wine.webp"},
  add: { name:"增幅", detail:"任意一只手数字 +1", target:"hand", image:"ink-mono/add.webp" },
  sub: { name:"退化", detail:"任意一只手数字 −1", target:"hand", image:"ink-mono/sub.webp" },
  lock: { name:"封印", detail:"封印一只手，使其不能参与计算", rules:"可以对任意玩家使用，至该玩家回合结束解除。每人最多封印一只手；道具仍能改变被封印手的数字。", target:"hand", image:"ink-mono/lock.webp" },
  civil: { name:"内战", detail:"所选手减去同一玩家另一只手的数字", rules:"可以对任意玩家使用。结果为负数时加 10；例如 [2] − [7] 得到 [5]。", target:"hand", image:"ink-mono/civil.webp" },
...
export const PROP_WEIGHTS = Object.freeze(Object.fromEntries(PROP_IDS.map(id =>
  [id, ['double', 'add', 'sub'].includes(id) ? 4 : ['grace', 'greed'].includes(id) ? 1 : 2]
)));
