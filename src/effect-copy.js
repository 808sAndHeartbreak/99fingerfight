...
export function effectDetail(old,next,command) {
 ...
 if(command.type==='prop') {
  ...
  const details={echo:`${team(a)}下一次计算复制到己方双手`,mirror:`${team(a)}下一次计算写入${team(b)}目标手，己方不变`,silence:`${team(b)}下个回合不能使用道具，仍可计算和合成`,wine:`${team(a)}下次直接攻击首段 +${p.wine*10} 伤害，出手后消耗`,grace:next.players.map((player,o)=>`${team(o)}回复 ${player.hp-old.players[o].hp} HP（${old.players[o].hp} → ${player.hp}）`).join("；"),ruin:`双方数字总和之差 |${old.players[a].hands.reduce((x,y)=>x+y,0)} − ${old.players[b].hands.reduce((x,y)=>x+y,0)}|；${team(b)}实际受到 ${old.players[b].hp-e.hp} 点真实伤害（HP ${old.players[b].hp} → ${e.hp}）`,greed:`${team(a)}获得 ${Math.max(0,p.props.length-old.players[a].props.length+1)} 个道具，双手归 [1]${next.active!==a?'，本回合结束':''}`};
 ...
 const buffs=[['knuckles','指虎','层'],['wine','酒','层'],['peace','和平','回合'],['resilience','坚韧','回合']].flatMap(([key,label,unit])=>Number(p[key])>Number(old.players[a][key])?[`${label} +${Number(p[key])-Number(old.players[a][key])}${unit}`]:[]);
 if(old.options?.itemsEnabled===false) {
   if(['fan','dual'].includes(id))return '';
   if(id==='steal')return buffs.length?`${team(a)}夺取增益：${buffs.join('、')}`:'没有可转移的增益';
 }
