...
export function beforeSupply(state) {
 const visual=structuredClone(state);
 for(const event of [...(state.events||[])].reverse())if(event.type==='draw'&&event.source==='回合补给'){
  const props=visual.players[event.owner].props,index=props.lastIndexOf(event.item);
...
export function actionDuration(old,next,c) {
 ...
 return actionBeats(old,next,c).length?900*actionBeats(old,next,c).length+1200:stateChanges(old,beforeSupply(next)).length?900:0;
}
