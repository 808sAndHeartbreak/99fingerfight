export function createBattleSound(enabled, audioSettings) {
 ...
 function play(kind,volume=1){
  ...
  const t=ctx.currentTime,tonal=['charge','nine','victory','guard','heal','item','turn'].includes(kind),dur=kind==='victory'?.85:kind==='charge'?.5:kind==='heal'?.42:kind==='release'?.32:.22;
